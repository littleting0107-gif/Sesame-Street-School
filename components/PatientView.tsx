...
export const PatientView: React.FC<PatientViewProps> = ({ bookings, onAddBooking }) => {
        ...
        <div className="w-full bg-gray-50 rounded-lg p-4 mb-6 space-y-2">
            ...
            {selectedSlots.sort((a,b) => a.date.localeCompare(b.date) || a.timeId.localeCompare(b.timeId)).map((slot, idx) => (
                <div key={idx} className="flex justify-between text-sm">
                    ...
                    <span className="font-bold text-teal-600">電腦 {slot.computerId}</span>
                </div>
            ))}
        </div>
                                        ...
                                        className={`
                                            ...
                                            ${occupied 
                                                ...
                                                : selected
                                                    ...
                                                    : 'bg-white border-2 border-teal-100 text-teal-600 hover:border-teal-400 hover:bg-teal-50'
                                            }
                                        `}
                                    >
                                        {comp}
  ...
  return (
    <div className="max-w-4xl mx-auto p-4 md:p-6 bg-white rounded-2xl shadow-xl border-t-4 border-teal-600 min-h-[80vh]">
      ...
      {/* Input Section */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6 bg-gray-50 p-4 rounded-xl">
        <div>
            <label className="flex items-center text-sm font-semibold text-gray-700 mb-1">
            <User className="w-4 h-4 mr-2" /> 姓名
            </label>
            <input
                type="text"
                value={name}
        ...
        <div>
            <label className="flex items-center text-sm font-semibold text-gray-700 mb-1">
            <GraduationCap className="w-4 h-4 mr-2" /> 班級
            </label>
            <input
                type="text"
                value={studentClass}
      ...
      <div className="flex flex-col md:flex-row gap-6">
        ...
        <div className="w-full md:w-1/2">
            <div className="bg-white border rounded-xl overflow-hidden shadow-sm">
                <div className="bg-teal-50 p-4 flex justify-between items-center">
                    <button onClick={handlePrevMonth} className="p-1 hover:bg-teal-100 rounded-full"><ChevronLeft className="w-5 h-5 text-teal-700"/></button>
                    <span className="font-bold text-teal-900 text-lg">{year}年 {month + 1}月</span>
                    <button onClick={handleNextMonth} className="p-1 hover:bg-teal-100 rounded-full"><ChevronRight className="w-5 h-5 text-teal-700"/></button>
                </div>
                        ...
                        return (
                            <button
                                ...
                                className={`
                                    ...
                                    ${isToday && !isSelectedDate ? 'border border-teal-400 font-bold text-teal-600' : ''}
                                `}
                            >
                                {day}
                                {hasUserSelection && (
            ...
            {selectedSlots.length > 0 && (
                <div className="mt-4 bg-orange-50 p-4 rounded-xl border border-orange-100">
                    <h3 className="font-bold text-orange-800 text-sm mb-2">已選擇的時段 ({selectedSlots.length})</h3>
                    <div className="flex flex-wrap gap-2">
                        {selectedSlots.map((s, i) => (
                            <span key={i} className="inline-flex items-center gap-1 bg-white border border-orange-200 text-orange-700 px-2 py-1 rounded text-xs font-bold shadow-sm">
                    ...
                    <button 
                        ...
                        className="mt-4 w-full bg-teal-600 hover:bg-teal-700 text-white font-bold py-3 rounded-lg shadow transition-colors flex items-center justify-center gap-2"
                    >
